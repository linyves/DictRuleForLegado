# Legado字典规则

导入方式：复制链接，Legado字典规则页面→网络导入


### 汉典

```
https://cdn.jsdelivr.net/gh/linyves/DictRuleForLegado@refs/heads/main/zdic
```

### 中文维基百科

```
https://cdn.jsdelivr.net/gh/linyves/DictRuleForLegado@refs/heads/main/zhwiki
```

### 能不能好好说话

[网站链接](https://lab.magiconch.com/nbnhhsh/)

【推荐】自动排序版

*释义按从短到长排序显示，在查询部分缩写时会提高易读性*

```
https://cdn.jsdelivr.net/gh/linyves/DictRuleForLegado@refs/heads/main/nbnhhsh_sort
```

无排序版

```
https://cdn.jsdelivr.net/gh/linyves/DictRuleForLegado@refs/heads/main/nbnhhsh
```
